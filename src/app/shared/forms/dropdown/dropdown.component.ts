...
export class DropdownComponent<T>
  extends AbstractInput<T | T[keyof T] | (T | T[keyof T])[] | null>
  implements OnInit, OnDestroy
{
  ...
  get parsedValue() {
    if (this.controlValue && this.optionValue) {
      return this.options.find(
        (option) => option[this.optionValue as keyof T] === this.controlValue
      );
    }
  ...
  public isOptionSelected(option: T) {
    if (this.type === 'single') {
      ...
      return this.controlValue
        ? (this.controlValue as (T | T[keyof T])[]).includes(
            this.transform(option, this.optionLabel)
          )
        : false;
    }
  }
  ...
  private selectMultiOption(option: T) {
    ...
    const newOptions = selectedOptions.includes(
      this.transform(option, this.optionValue)
    )
      ? selectedOptions.filter(
          (values) => values !== this.transform(option, this.optionValue)
        )
